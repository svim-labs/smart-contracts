...
contract Pool is IPool, ERC20Upgradeable, BeaconImplementation {
  ...
  modifier onlyPoolController() {
    require(
      address(poolController) != address(0) && msg.sender == address(poolController),
  ...
  modifier atState(IPoolLifeCycleState state_) {
    require(poolController.state() == state_, 'Pool: FunctionInvalidAtThisLifeCycleState');
    _;
  }
  ...
  function initialize(
    ...
    _feeVault = IVault(IVaultFactory(vaultFactory).createVault(address(this)));

    // Build the withdraw controller
    withdrawController = IWithdrawController(
      IWithdrawControllerFactory(withdrawControllerFactory).createController(address(this))
    );
   ...
   * @inheritdoc IPool
   */
  function serviceConfiguration() public view returns (IServiceConfiguration) {
   ...
   * @inheritdoc IPool
   */
  function settings() public view returns (IPoolConfigurableSettings memory poolSettings) {
   ...
   * @inheritdoc IPool
   */
  function accountings() external view override returns (IPoolAccountings memory) {
    return _accountings;
  }
  ...
  function onActivated() external onlyPoolController {
    ...
    if (_settings.fixedFee != 0) {
      _accountings.fixedFeeDueDate = block.timestamp + _settings.fixedFeeInterval * 1 days;
    }
  }
  ...
  function onLoanStateTransitioned() external override {
    ...
    if (loanState == ILoanLifeCycleState.Matured || loanState == ILoanLifeCycleState.Defaulted) {
   ...
   * @inheritdoc IPool
   */
  function onLoanDefaulted(address loan, uint256 firstLossApplied) external override onlyPoolController {
  ...
  function totalAvailableSupply() public view override returns (uint256 shares) {
    shares = PoolLib.calculateTotalAvailableShares(address(this), withdrawController.totalRedeemableShares());
  }

  /**
   * @inheritdoc IPool
   */
  function currentExpectedInterest() external view override returns (uint256) {
  ...
  function claimFixedFee(
    ...
    require(_accountings.fixedFeeDueDate < block.timestamp, 'Pool: fixed fee not due');
  ...
  function _performRedeemRequest(address owner, uint256 shares, uint256 assets) internal {
    require(withdrawController.maxRedeemRequest(owner) >= shares, 'Pool: InsufficientBalance');
  ...
  function _performRequestCancellation(address owner, uint256 shares, uint256 assets) internal {
    require(maxRequestCancellation(owner) >= shares, 'Pool: InsufficientBalance');
  ...
  function _performSnapshot() internal {
    (uint256 period, uint256 redeemableShares, uint256 withdrawableAssets, bool periodSnapshotted) = withdrawController
      .snapshot(poolController.withdrawGate());
  ...
  function totalAssets() public view returns (uint256) {
    return
      PoolLib.calculateTotalAssets(address(_liquidityAsset), address(this), _accountings.outstandingLoanPrincipals);
  }
  ...
  function convertToShares(uint256 assets) public view override returns (uint256 shares) {
    shares = PoolLib.calculateSharesFromAssets(assets, totalAvailableSupply(), totalAvailableAssets(), false);
  }
  ...
  function convertToAssets(uint256 shares) public view override returns (uint256 assets) {
    assets = PoolLib.calculateAssetsFromShares(shares, totalAvailableAssets(), totalAvailableSupply(), false);
  }
  ...
  function maxDeposit(address) public view virtual override returns (uint256) {
    ...
    return PoolLib.calculateMaxDeposit(poolController.state(), settings().maxCapacity, totalAvailableAssets());
  }
  ...
  function previewDeposit(uint256 assets) public view override returns (uint256 shares) {
    shares = PoolLib.calculateSharesFromAssets(
      ...
      totalAvailableAssets() + PoolLib.calculateExpectedInterest(_activeLoans),
      false
    );
  }
