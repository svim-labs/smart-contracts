...
contract PoolController is IPoolController, BeaconImplementation {
  ...
  modifier onlyNotPaused() {
    require(IServiceConfiguration(serviceConfiguration).paused() == false, 'Pool: Protocol paused');
    _;
  }
  ...
  modifier onlyPermittedAdmin() virtual {
    // TODO: Add a new mapping and related functions; use it to extend pool admin roles to the broader dev team. -------Sean
  ...
  modifier isPoolLoan(address loan) {
    require(PoolLib.isPoolLoan(loan, serviceConfiguration, address(pool)), 'Pool: invalid loan');
    _;
  }
  ...
  function initialize(
    ...
    _firstLossVault = IVault(IVaultFactory(vaultFactory).createVault(address(this)));
  ...
  function setRequestFee(
    uint256 feeBps
  ) external onlyNotPaused onlyPermittedAdmin onlyAdmin atState(IPoolLifeCycleState.Initialized) {
  ...
  function requestFee(uint256 sharesOrAssets) public view returns (uint256 feeShares) {
    feeShares = PoolLib.calculateRequestFee(sharesOrAssets, _settings.requestFeeBps);
  }
  ...
  function requestCancellationFee(uint256 sharesOrAssets) public view returns (uint256 feeShares) {
    feeShares = PoolLib.calculateCancellationFee(sharesOrAssets, _settings.requestCancellationFeeBps);
  }
  ...
  ) external onlyNotPaused onlyPermittedAdmin onlyAdmin atInitializedOrActiveState {
   ...
   * @inheritdoc IPoolController
   */
  function setServiceFeeBps(uint256 serviceFeeBps) external onlyNotPaused onlyPermittedAdmin onlyAdmin {
   ...
   * @inheritdoc IPoolController
   */
  function setFixedFee(uint256 amount, uint256 interval) external onlyNotPaused onlyPermittedAdmin onlyAdmin {
    if (amount > 0) {
  ...
  function isInitializedOrActive() external view returns (bool) {
    ...
    return _currentState == IPoolLifeCycleState.Initialized || _currentState == IPoolLifeCycleState.Active;
  }
  ...
  function isActiveOrClosed() external view returns (bool) {
    ...
    return _currentState == IPoolLifeCycleState.Active || _currentState == IPoolLifeCycleState.Closed;
  }
  ...
  function depositFirstLoss(
    ...
    _liquidityAsset.safeTransferFrom(spender, address(_firstLossVault), amount);

    IPoolLifeCycleState poolLifeCycleState = PoolLib.executeFirstLossDeposit(
      pool.asset(),
      spender,
      amount,
      pool.firstLossVault(),
  ...
  function withdrawFirstLoss(
    uint256 amount,
    address receiver
  ) external onlyNotPaused onlyPermittedAdmin onlyAdmin atState(IPoolLifeCycleState.Closed) returns (uint256) {
    require(pool.numActiveLoans() == 0, 'Pool: loans still active');
    require(address(_firstLossVault) != address(0), 'Pool: 0 address');
    require(receiver != address(0), 'Pool: 0 address');

    _firstLossVault.withdrawERC20(address(_liquidityAsset), amount, receiver);

    return PoolLib.executeFirstLossWithdraw(amount, receiver, pool.firstLossVault());
  }
  ...
  function defaultLoan(
    ...
    PoolLib.executeDefault(address(_liquidityAsset), address(_firstLossVault), loan, address(pool));
  }
  ...
  function reclaimLoanFunds(
    address loan,
    uint256 amount
  ) external override onlyNotPaused onlyAdmin onlyPermittedAdmin onlySnapshottedPool {
  ...
  function claimLoanCollateral(
    ...
    ILoan(loan).claimCollateral(assets);
  }
   ...
   * @dev Note that the Loan enforces the non-paused state, so it's omitted here.
   */
  function cancelFundedLoan(address loan) external override onlyAdmin onlyPermittedAdmin onlySnapshottedPool {
   ...
   * @inheritdoc IPoolController
   */
  function claimFixedFee() external onlyNotPaused onlyPermittedAdmin onlyAdmin {
    pool.claimFixedFee(msg.sender, _settings.fixedFee, _settings.fixedFeeInterval);
  }
   ...
   * @inheritdoc IPoolController
   */
  function snapshot() external override onlyNotPaused onlyPermittedAdmin onlyAdmin {
    pool.snapshot();
  }
}
