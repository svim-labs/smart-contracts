...
contract WithdrawController is IWithdrawController, BeaconImplementation {
   ...
   */
  function _currentWithdrawState(address owner) internal view returns (IPoolWithdrawState memory state) {
    uint256 currentPeriod = withdrawPeriod();
    state = PoolLib.progressWithdrawState(_withdrawState[owner], currentPeriod);
  }

  /**
   * @dev Returns the current global withdraw state.
   */
  function _currentGlobalWithdrawState() internal view returns (IPoolWithdrawState memory state) {
    state = PoolLib.progressWithdrawState(_globalWithdrawState, withdrawPeriod());
  }
  ...
  function previewRedeemRequestFees(uint256 shares) external view returns (uint256 feeShares) {
    feeShares = PoolLib.calculateRequestFee(shares, _pool.settings().requestFeeBps);
  }
  ...
  function previewWithdrawRequestFees(uint256 assets) external view returns (uint256 feeShares) {
    uint256 assetFees = PoolLib.calculateRequestFee(assets, _pool.settings().requestFeeBps);
  ...
  function performRequest(address owner, uint256 shares) external onlyPool {
    ...
    require(!_claimRequired(_state), 'WithdrawController: must claim snapshots first');

    uint256 currentPeriod = withdrawPeriod();

    // Update the requested amount from the user
    _withdrawState[owner] = PoolLib.calculateWithdrawStateForRequest(_state, currentPeriod, shares);
    ...
    _withdrawState[owner].latestSnapshotPeriod = _globalWithdrawState.latestSnapshotPeriod;

    // Update the global amount
    _globalWithdrawState = PoolLib.calculateWithdrawStateForRequest(_globalWithdrawState, currentPeriod, shares);
  }
  ...
  function maxRequestCancellation(address owner) public view returns (uint256 maxShares) {
    maxShares = PoolLib.calculateMaxCancellation(_currentWithdrawState(owner));
  }

  /**
   * @inheritdoc IWithdrawController
   */
  function performRequestCancellation(address owner, uint256 shares) external onlyPool {
    require(!claimRequired(owner), 'WithdrawController: must claim eligible first');
    uint256 currentPeriod = withdrawPeriod();
    ...
    lenderState = PoolLib.calculateWithdrawStateForCancellation(lenderState, shares);
    ...
    _globalWithdrawState = PoolLib.progressWithdrawState(_globalWithdrawState, currentPeriod);
    _globalWithdrawState.requestedShares -= requestedSharesPrior - lenderState.requestedShares;
    _globalWithdrawState.eligibleShares -= eligibleSharesPrior - lenderState.eligibleShares;
  }
  ...
  function snapshot(
    ...
    external
    onlyPool
    returns (uint256 period, uint256 redeemableShares, uint256 withdrawableAssets, bool periodSnapshotted)
  {
    ...
    uint256 availableAssets = liquidAssets.mul(withdrawGate).mul(RAY).div(10_000).div(RAY);
    ...
    redeemableShares = Math.min(availableShares, globalState.eligibleShares);
    ...
    _snapshots[period] = IPoolSnapshotState(
      redeemableRateRay,
      redeemableShares,
      redeemableShares > 0 ? withdrawableAssets.mul(RAY).div(redeemableShares) : 0,
      0
    );

    // Update the global withdraw state to earmark those funds
    globalState = PoolLib.updateWithdrawStateForWithdraw(globalState, withdrawableAssets, redeemableShares);

    // "Point" the prior snapshot to this one
    _snapshots[globalState.latestSnapshotPeriod].nextSnapshotPeriod = period;
   ...
   */
  function _claimRequired(IPoolWithdrawState memory state) internal view returns (bool) {
    return state.eligibleShares > 0 && state.latestSnapshotPeriod < _globalWithdrawState.latestSnapshotPeriod;
  }
  ...
  function claimSnapshots(
    address lender,
    uint256 limit
  ) external onlyPool returns (uint256 sharesRedeemable, uint256 assetsWithdrawable) {
    require(limit > 0, 'WithdrawController: invalid limit');
    IPoolWithdrawState memory withdrawState = _currentWithdrawState(lender);
    uint256 periodToClaim = _snapshots[withdrawState.latestSnapshotPeriod].nextSnapshotPeriod;
    ...
    for (snapshotsClaimed; snapshotsClaimed <= limit; snapshotsClaimed++) {
      ...
      snapshotShares = withdrawState.eligibleShares.mul(_periodSnapshot.redeemableRateRay).div(RAY);

      withdrawState.eligibleShares -= snapshotShares;
      sharesRedeemable += snapshotShares;
      assetsWithdrawable += snapshotShares.mul(_periodSnapshot.fxRateRay).div(RAY);
    ...
    emit SnapshotsClaimed(lender, snapshotsClaimed, sharesRedeemable, assetsWithdrawable);
  }
  ...
  function redeem(address owner, uint256 shares) external onlyPool returns (uint256 assets) {
    ...
    assets = PoolLib.calculateAssetsFromShares(shares, state.withdrawableAssets, state.redeemableShares, false);
  ...
  function withdraw(address owner, uint256 assets) external onlyPool returns (uint256 shares) {
    ...
    shares = PoolLib.calculateSharesFromAssets(assets, state.redeemableShares, state.withdrawableAssets, true);
  ...
  function _performWithdraw(
    ...
    require(assets <= currentState.withdrawableAssets, 'Pool: InsufficientBalance');

    require(shares <= currentState.redeemableShares, 'Pool: InsufficientBalance');
