...
library PoolLib {
  ...
  function isPoolLoan(address loan, address serviceConfiguration, address pool) public view returns (bool) {
    ...
    return
      IServiceConfiguration(serviceConfiguration).isLoanFactory(factory) &&
  ...
  function executeFirstLossDeposit(
    ...
    if (
      ...
      (amount >= minFirstLossRequired ||
        IERC20(liquidityAsset).balanceOf(address(firstLossVault)) >= minFirstLossRequired)
    ) {
  ...
  function calculateExpectedInterest(
    ...
    for (uint256 i = 0; i < activeLoans.length(); i++) {
      ...
      numberPaymentsLate = paymentDueDate < block.timestamp
        ? Math.min((block.timestamp - paymentDueDate) / paymentPeriod, paymentsRemaining)
        : 0;
      ...
      expectedInterest += paymentAmount.mul(RAY).mul(block.timestamp - paymentPeriodStart).div(paymentPeriod).div(RAY);
    }
  ...
  function calculateTotalAssets(
    ...
    totalAssets = IERC20(asset).balanceOf(vault) + outstandingLoanPrincipals;
  }
  ...
  function calculateTotalAvailableAssets(
    ...
    totalAvailableAssets = calculateTotalAssets(asset, vault, outstandingLoanPrincipals) - withdrawableAssets;
  }
  ...
  function calculateMaxDeposit(
    ...
    return poolLifeCycleState == IPoolLifeCycleState.Active ? remainingCapacity : 0;
  }
  ...
  function executeDefault(address asset, address firstLossVault, address loan, address pool) external {
    ...
    uint256 firstLossBalance = IERC20(asset).balanceOf(address(firstLossVault));

    uint256 outstandingLoanDebt = ILoan(loan).outstandingPrincipal();
    uint256 firstLossRequired = firstLossBalance >= outstandingLoanDebt ? outstandingLoanDebt : firstLossBalance;
  ...
  function calculateMaxRedeemRequest(
    ...
    uint256 sharesRemaining = shareBalance - state.requestedShares - state.eligibleShares - state.redeemableShares;
