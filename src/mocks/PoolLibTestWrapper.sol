...
contract PoolLibTestWrapper is ERC20('PoolLibTest', 'PLT') {
  ...
  event FirstLossDeposited(address indexed caller, address indexed supplier, uint256 amount);
  ...
  function executeFirstLossWithdraw(
    ...
    return PoolLib.executeFirstLossWithdraw(amount, withdrawReceiver, firstLossVault);
  }
  ...
  function calculateConversion(
    ...
    return PoolLib.calculateConversion(input, numerator, denominator, roundUp);
  }
  ...
  function calculateSharesFromAssets(
    ...
    return PoolLib.calculateSharesFromAssets(assets, totalShares, totalAssets, roundUp);
  }
  ...
  function calculateAssetsFromShares(
    ...
    return PoolLib.calculateAssetsFromShares(shares, totalAssets, totalShares, roundUp);
  }
  ...
  function calculateTotalAssets(
    ...
    return PoolLib.calculateTotalAssets(asset_, vault, outstandingLoanPrincipals);
  }
  ...
  function calculateTotalAvailableAssets(
    ...
    return PoolLib.calculateTotalAvailableAssets(asset_, vault, outstandingLoanPrincipals, withdrawableAssets);
  }
  ...
  function calculateMaxDeposit(
    ...
    return PoolLib.calculateMaxDeposit(poolLifeCycleState, poolMaxCapacity, totalAvailableAssets);
  }
  ...
  function executeDeposit(
    ...
    return PoolLib.executeDeposit(asset_, vault, sharesReceiver, assets, shares, maxDeposit, _mint, _accountings);
  }
  ...
  function calculateCurrentWithdrawPeriod(
    ...
    return PoolLib.calculateCurrentWithdrawPeriod(currentTimestamp, activatedAt, withdrawalWindowDuration);
  }
  ...
  function calculateWithdrawStateForRequest(
    ...
    return PoolLib.calculateWithdrawStateForRequest(state, currentPeriod, requestedShares);
  }
  ...
  function calculateWithdrawStateForCancellation(
    ...
    return PoolLib.calculateWithdrawStateForCancellation(state, cancelledShares);
  }
  ...
  function calculateCancellationFee(uint256 shares, uint256 requestCancellationFeeBps) external pure returns (uint256) {
    return PoolLib.calculateCancellationFee(shares, requestCancellationFeeBps);
  }
  ...
  function calculateMaxRedeemRequest(
    ...
    return PoolLib.calculateMaxRedeemRequest(state, shareBalance, requestFeeBps);
  }

  function calculateMaxCancellation(IPoolWithdrawState memory state) public pure returns (uint256) {
